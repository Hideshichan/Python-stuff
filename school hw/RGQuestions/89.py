...
if (len (passKey) == 9):
    ...
    symbolLocation =locateSpecial(passKey)
    ...
    if (symbolLocation != -1):
        ...
        for index in range (1, symbolLocation):
            temp = temp + passKey[index]

        # =====> Complete the values for the range() function to
        #        copy all characters after the special symbol
        #        to a temporary variable
        for index in range (symbolLocation + 1, len(passKey)):
            temp = temp + passKey[index]
